...
class Larry(Monster):
    ...
    filename = 'data/images/mutants/larry/left.png'
...
class Kissy(Monster):
    ...
    filename = 'data/images/mutants/kissy/kissy.png'
    ...
    def choose_aim_position(self):
        if self.state == self.HUNT:
            ...
            self.aim_x, self.aim_y = player.rect.x + 18 * sucmen_directions[player.direction][0], \
                                     player.rect.y + 18 * sucmen_directions[player.direction][1]
        else:
            super().choose_aim_position()
...
class Huggy(Monster):
    ...
    IMAGES = {Monster.LEFT: 'data/images/mutants/huggy/left.png',
              Monster.UP: 'data/images/mutants/huggy/up.png',
              Monster.DOWN: 'data/images/mutants/huggy/down.png',
              Monster.RIGHT: 'data/images/mutants/huggy/right.png'}

    def __init__(self, game, field, x=10 + 18 * 14, y=10 + 18 * 13):
        super().__init__(self.filename, game, field, x, y)
        self.x, self.y = x, y
        self.aim_default_x, self.aim_default_y = 10 - 18 * 50, 10 - 18 * 50
        self.aim_begin_x, self.aim_begin_y = 10 + 18 * 13, 10 + 18 * 11
    ...
    def choose_aim_position(self):
        if self.state == self.HUNT:
            ...
            self.aim_x, self.aim_y = player.rect.x + 18 * sucmen_directions[player.direction][0], \
                                     player.rect.y + 18 * sucmen_directions[player.direction][1]
            delta = [(player.rect.x + 18 * sucmen_directions[player.direction][0] - larry.rect.x) * 2,
                     (player.rect.x + 18 * sucmen_directions[player.direction][1] - larry.rect.y) * 2]

            self.aim_x, self.aim_y = larry.rect.x + delta[0], larry.rect.y + delta[1]
        else:
            super().choose_aim_position()
...
class Wunk(Monster):
    """ Ванк Панк = Клайд (оранжевый призрак) из оригинальной игры) """
    filename = 'data/images/mutants/wunk/wunk.png'
    IMAGES = {Monster.LEFT: 'data/images/mutants/wunk/left.png',
              Monster.UP: 'data/images/mutants/wunk/up.png',
              Monster.DOWN: 'data/images/mutants/wunk/down.png',
              Monster.RIGHT: 'data/images/mutants/wunk/right.png'}
